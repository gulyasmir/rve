...
<div class="proect-form">
    ...
    <?php
      if(isset($model->img_to_page_index) && file_exists(Yii::getAlias('@webroot', $model->img_to_page_index)))
      {
          echo Html::img($model->img_to_page_index, ['class'=>'img-responsive']);
        //  echo $form->field($model,'del_img1')->checkBox(['class'=>'span-1']);
      }
  ?>
  <?= $form->field($model, 'file_img_to_page_index')->fileInput()->label('Фото для главной страницы ') ?>
    ...
    <?= $form->field($model, 'adress')->widget(TinyMce::className(), [
            'options' => ['rows' => 6],
            'language' => 'ru',
            'clientOptions' => [
            'plugins' => [
